...
====================
ResNet50 transfer learning — two-phase fine-tuning on PlantVillage.

Phase 1 : freeze backbone, train new classifier head only  (10 epochs, patience 4)
Phase 2 : unfreeze layer3 + layer4 + head, differential LRs (20 epochs, patience 6)

Checkpoint / resume : each phase saves its own checkpoint so the job can be
killed and restarted at any point without losing progress.

Usage
-----
  python 05_model_resnet50.py            # start / resume automatically
  python 05_model_resnet50.py --reset    # ignore checkpoints, start fresh
"""
...
separator("=")
log("  05 — ResNet50 Transfer Learning  (2-phase fine-tuning)")
separator("=")
...
BATCH_SIZE   = 256   # ResNet50 is heavier than CustomCNN; 256 is safe on A100 80GB
_PREFETCH    = 4
...
def build_resnet50(n_classes, dropout=0.5):
...
def run_phase(phase_name, ckpt_path, optimizer, scheduler,
    ...
    if history is not None and not hist["train_loss"]:
        hist = history   # carry history from previous phase on fresh start
...
log(f"Phase 2 — Trainable parameters : {count_trainable(model):,}  (layer3+layer4+head)")
...
plt.tight_layout()
curves_path = os.path.join(RESULTS_DIR, f"05_{MODEL_NAME}_curves.png")
...
plt.tight_layout()
cm_path = os.path.join(RESULTS_DIR, f"05_{MODEL_NAME}_confusion_matrix.png")
...
log()
log("  Next: 06_model_efficientnet.py")
separator("=")
