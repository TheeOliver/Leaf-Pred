...
========================
EfficientNet-B2 transfer learning — two-phase fine-tuning on PlantVillage.

Phase 1 : freeze backbone, train classifier head only     (10 epochs, patience 4)
Phase 2 : unfreeze features[6,7,8] + head, diff LRs      (20 epochs, patience 6)

Usage
-----
  python 06_model_efficientnet.py            # start / resume automatically
  python 06_model_efficientnet.py --reset    # ignore checkpoints, start fresh
"""
...
separator("=")
log("  06 — EfficientNet-B2 Transfer Learning  (2-phase fine-tuning)")
separator("=")
...
BATCH_SIZE   = 256   # EfficientNet-B2 is lighter than ResNet50; 256 is comfortable on A100
_PREFETCH    = 4
...
def build_efficientnet_b2(n_classes, dropout=0.4):
    m = models.efficientnet_b2(weights=models.EfficientNet_B2_Weights.IMAGENET1K_V1)
    in_features = m.classifier[1].in_features  # 1408
...
def unfreeze_features(model, block_indices):
...
model = build_efficientnet_b2(N_CLASSES).to(DEVICE)
...
unfreeze_features(model, block_indices=[6, 7, 8])
log(f"Phase 2 — Trainable parameters : {count_trainable(model):,}  (blocks 6,7,8 + head)")
...
history, best_val_loss = run_phase(
    "Phase 2 — Fine-tuning (blocks 6,7,8 + head)",
...
plt.tight_layout()
curves_path = os.path.join(RESULTS_DIR, f"06_{MODEL_NAME}_curves.png")
...
plt.tight_layout()
cm_path = os.path.join(RESULTS_DIR, f"06_{MODEL_NAME}_confusion_matrix.png")
...
separator("=")
log("  06 — EfficientNet-B2 COMPLETE")
separator("=")
